...
const useCssHandles = <T extends CssHandlesList>(
  ...
  const values = useMemo<CssHandles<T>>(() => {
    const { migrationFrom, classes: handlesOverrides } = options
    ...
    if (handlesOverrides) {
      if (
        ...
        handlesOverrides.__classes !== SYMBOL_CUSTOM_CLASSES
      ) {
        throw new Error(
          '[css-handles] A custom classes object was passed that was not generated by the "useCustomClasses" hook.'
        )
      }

      Object.keys(handlesOverrides).forEach((handleName: ValueOf<T>) => {
